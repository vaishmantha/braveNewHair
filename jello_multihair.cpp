...
struct world jelloList[numHairs]; 
...
void doIdle()
{
  ...
  if (pause == 0)
  {
    ...
    for(int i = 0; i < numHairs; i++){
      for(int j = 0; j < numHairs; j++){
        if (i != j){
          Euler(&jelloList[i], &jelloList[j]); 
          Euler(&jelloList[j], &jelloList[i]); 
        }
      }
    }
  }
...
int main (int argc, char ** argv)
{
  ...
  // readWorld(argv[1], &jello[0], 0.0, 0.0);
  // readWorld(argv[1],&jello2, 0.0, strandOffset);
  
  // jello.pushedByWind  = true;
  // jello2.pushedByWind = false;

  int side = int(sqrt(numHairs));
  for(int j = 0; j < side; j++){
    for(int i = 0; i < side; i++){
      double shift = (strandOffsetMulti * numHairs) / 2; 
      int index = i * side + j; 
      readWorld(argv[1], &jelloList[index], i * strandOffsetMulti - shift, j * strandOffsetMulti - shift);
      jelloList[index].pushedByWind = true; 
    }
  }
